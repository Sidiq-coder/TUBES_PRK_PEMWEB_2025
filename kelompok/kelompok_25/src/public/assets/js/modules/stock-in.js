...
const StockInModule = {
        ...
        tbody.innerHTML = this.state.stockIns.map((item, index) => {
            ...
            return `
                <tr class="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td class="py-3 px-4">
                        <button onclick="StockInModule.viewDetail(${item.id})" class="text-blue-600 hover:text-blue-700 mr-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                            </svg>
                        </button>
                        <button onclick="StockInModule.deleteStockIn(${item.id})" class="text-red-600 hover:text-red-700">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    async deleteStockIn(id) {
        if (!confirm('Apakah Anda yakin ingin menghapus stok masuk ini?')) {
            return;
        }
